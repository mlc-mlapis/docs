...
It's possible to distinguish between system messages generated directly by the runtime environment, and messages generated by application code. All messages sent to **Stderr** (standard error file descriptor) or **Stdout** (standard output file descriptor), like by using the Node.js `console` module, are processed via **Linux Systemd daemon** as log messages with the **facility number 16** (local0) and **severity 6** (Informational) by default (see [RFC5424](https://datatracker.ietf.org/doc/html/rfc5424#section-6.2.1)).
...
* Those methods are also synchronous functions when the destination is a terminal or a file, so they are not suitable for production very well unless you pipe the output to another program.

* For production app activity logging, using logging libraries like [winston](https://www.npmjs.com/package/winston), [bunyan](https://www.npmjs.com/package/bunyan), or [syslog-client](https://www.npmjs.com/package/syslog-client) would probably be more performant. You can also set up facility numbers, severities, and other parameters more flexibly.
