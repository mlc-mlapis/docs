...
It's possible to distinguish between system messages generated directly by the runtime environment, including web server logs, and messages generated by user application code. All messages sent to **Stderr** (standard error file descriptor) or **Stdout** (standard output file descriptor), like by using the Golang `fmt` package, are processed via **Linux Systemd daemon** as log messages with the **facility number 16** (local0) and **severity 6** (Informational) by default (see [RFC5424](https://datatracker.ietf.org/doc/html/rfc5424#section-6.2.1)).
...
Then you can filter log messages generated by user application code by selecting the severity level, or show all of them by selecting the **All** option.
