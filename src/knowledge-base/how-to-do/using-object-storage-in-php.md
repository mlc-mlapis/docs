...
When having `$credentials` from the previous code snippet you can create a named bucket. Zerops uses multi-tenancy feature

function createTheBucket($credentials) {
		$s3 = new S3Client([
			'version' => 'latest',
			'region' => 'us-east-1',
		...
		echo createBucket($s3, 'uploads-demo');
	}
