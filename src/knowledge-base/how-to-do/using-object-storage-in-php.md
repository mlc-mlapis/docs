...
At this time, environment variables generated by Zerops Object Storage Services are **not available** in other services. For this reason, it is necessary to use their `accessKeyId` (equal to **serviceId** and **user's canonical ID**), `secretAccessKey`, or `apiUrl` values by manually copying them from the [environment variables](/documentation/environment-variables/how-to-access.html) card of the service details.
...
The code for getting ==**$uniqueBucketPrefix**== (a unique prefix for a bucket name):

```php
$uniqueBucketPrefix = '0LW7E_BUT-yKmmEcyf3dzQ';
```
